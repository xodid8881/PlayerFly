...
class MainCommand(private val plugin: PlayerFlyPlugin) : CommandExecutor {
    ...
    private fun handlePlayer(player: Player, args: Array<out String>): Boolean {
        ...
        return when (args[0].lowercase()) {
            "정보" -> info(player, args)
            "지급" -> changeMinutes(player, args, +1)
            "차감" -> changeMinutes(player, args, -1)
            "설정" -> setMinutes(player, args)
    ...
    private fun handleConsole(sender: CommandSender, args: Array<out String>): Boolean {
        if (args.isEmpty()) {
            sender.sendMessage("${plugin.prefix()} /플라이관리 [지급|차감|설정] <플레이어> <분>")
            return true
        }
        return when (args[0].lowercase()) {
            "지급" -> changeMinutes(sender, args, +1)
            "차감" -> changeMinutes(sender, args, -1)
            "설정" -> setMinutes(sender, args)
    ...
    private fun info(sender: CommandSender, args: Array<out String>): Boolean {
        ...
        sender.sendMessage("${plugin.prefix()} ${target.name}님의 남은 시간: ${data.minutes}분")
    ...
    private fun changeMinutes(sender: CommandSender, args: Array<out String>, direction: Int): Boolean {
        if (args.size < 3) {
            sender.sendMessage("${plugin.prefix()} /플라이관리 ${if (direction > 0) "지급" else "차감"} <플레이어> <분>")
            return true
        }
        val target = Bukkit.getPlayerExact(args[1]) ?: run {
            sender.sendMessage("${plugin.prefix()} 해당 플레이어가 접속 중이 아닙니다.")
        ...
        sender.sendMessage("${plugin.prefix()} ${target.name}님에게 ${amount}분 $action 완료. 현재 ${next}분")
    ...
    private fun setMinutes(sender: CommandSender, args: Array<out String>): Boolean {
        if (args.size < 3) {
            sender.sendMessage("${plugin.prefix()} /플라이관리 설정 <플레이어> <분>")
            return true
        }
        val target = Bukkit.getPlayerExact(args[1]) ?: run {
            sender.sendMessage("${plugin.prefix()} 해당 플레이어가 접속 중이 아닙니다.")
        ...
        sender.sendMessage("${plugin.prefix()} ${target.name}님의 플라이 시간을 ${amount}분으로 설정했습니다.")
    ...
    private fun reload(sender: CommandSender): Boolean {
        ...
        for (target in Bukkit.getOnlinePlayers()) {
            if (target.gameMode != GameMode.CREATIVE && target.gameMode != GameMode.SPECTATOR) {
    ...
    private fun sendHelp(sender: CommandSender) {
        sender.sendMessage("${plugin.prefix()} /플라이관리 정보 <플레이어>")
        sender.sendMessage("${plugin.prefix()} /플라이관리 지급 <플레이어> <분>")
        sender.sendMessage("${plugin.prefix()} /플라이관리 차감 <플레이어> <분>")
        sender.sendMessage("${plugin.prefix()} /플라이관리 설정 <플레이어> <분>")
